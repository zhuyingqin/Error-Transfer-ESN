...
for pop_indx = 1:config.pop_size
    ...
    population(pop_indx).test_sequence = 1;
    population(pop_indx).err_r2=1;
    population(pop_indx).err_rmse=1;
    ...
    for i = 1:config.num_reservoirs
        ...
        % define num of units 由于新构建的连接矩阵已经包含节点数目 删去
        % population(pop_indx).nodes(i) = config.num_nodes(i);

        
        % Scaling and leak rate 由于新构建的连接矩阵已经包含泄露率 删去
        % population(pop_indx).input_scaling(i) = rand()*0.05;   % increases nonlinearity
    ...
    for i= 1:config.num_reservoirs
        ...
        for j= 1:config.num_reservoirs
            ...
            if i == j
                ...
                % population(pop_indx).connectivity(i,j) = config.input_scaling; 
            else
            ...
            %
            internal_weights = internal_weights *  population(pop_indx).W_scaling(i,j) / max(abs(eigs(internal_weights))); 
            ...
            population(pop_indx).eigenvalue(i,j) = real(mean(abs(eigs(full(population(pop_indx).W{i,j})))));
            % 
            if i ~=j
    ...
    if config.evolve_feedback_weights
        population(pop_indx).feedback_scaling = rand();
        % population(pop_indx).feedback_scaling = config.input_scaling;
